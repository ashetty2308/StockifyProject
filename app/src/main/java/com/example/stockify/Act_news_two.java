...
public class Act_news_two extends AppCompatActivity {
    @Override
    ...
    private class GetArticleDetails extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPostExecute(Void aVoid) {
            ...
            floatingActionButton.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    try {
                        ...
                        databaseReference1.setValue(stringData.getJSONArray("results").getJSONObject(1).get("title").toString());
                    } catch (JSONException e) {
                        e.printStackTrace();
                    }
                }
            ...
            try {
                String imageURL = stringData.getJSONArray("results").getJSONObject(1).get("image_url").toString();
            } catch (JSONException e) {
                e.printStackTrace();
            }

            try {
                String captionStr = stringData.getJSONArray("results").getJSONObject(1).get("description").toString();
                if(!captionStr.equalsIgnoreCase("null")){
                    caption.setText("Caption: "+stringData.getJSONArray("results").getJSONObject(1).get("description").toString());
                }
                else{
            ...
            try {
                title.setText("Title: "+stringData.getJSONArray("results").getJSONObject(1).get("title").toString());
            } catch (JSONException e) {
                e.printStackTrace();
            }
            ...
            try {
                if(!stringData.getJSONArray("results").getJSONObject(1).get("image_url").toString().equals("null")){
                    Picasso.get().load(stringData.getJSONArray("results").getJSONObject(1).get("image_url").toString()).into(imageView);
                }
                else{
            ...
            try {
                String storyStr = stringData.getJSONArray("results").getJSONObject(1).get("content").toString();
                if(!storyStr.equalsIgnoreCase("null")){
                    story.setText("Article: "+stringData.getJSONArray("results").getJSONObject(1).get("content").toString());
                }
                else{
            ...
            try {
                continueReading.setText("Link to Article: " +stringData.getJSONArray("results").getJSONObject(1).get("link").toString());
            } catch (JSONException e) {
                e.printStackTrace();
            }
