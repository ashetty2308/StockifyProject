...
public class Act_news_five extends AppCompatActivity {
    ...
    private class GetArticleDetails extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPostExecute(Void aVoid) {
            ...
            try {
                title.setText("Title: "+stringData.getJSONArray("results").getJSONObject(4).get("title").toString());
            } catch (JSONException e) {
                e.printStackTrace();
            }
            ...
            try {
                if(!stringData.getJSONArray("results").getJSONObject(4).get("image_url").toString().equals("null")){
                    Picasso.get().load(stringData.getJSONArray("results").getJSONObject(4).get("image_url").toString()).into(imageView);
                }
                else{
